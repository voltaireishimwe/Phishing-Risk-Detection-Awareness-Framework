...
class PhishingFeatureEngineer:
    ...
    def extract_features(self, df):
        ...
        # Date features - FIXED: Handle multiple date formats
        features_df['hour_of_day'] = self._extract_hour(df['date'])
        features_df['day_of_week'] = self._extract_day_of_week(df['date'])
    ...
    def _extract_hour(self, date_series):
        """Extract hour from date with multiple format support"""
        hours = []
        for date_str in date_series:
            try:
                # Try multiple date formats
                if 'GMT' in str(date_str):
                    # Format: "Thu, 22 Aug 2002 18:57:35 GMT"
                    dt = datetime.strptime(str(date_str), '%a, %d %b %Y %H:%M:%S GMT')
                elif 'UTC' in str(date_str):
                    # Format with UTC
                    dt = datetime.strptime(str(date_str).replace('UTC', '').strip(), '%a, %d %b %Y %H:%M:%S')
                else:
                    # Try ISO format or other common formats
                    dt = pd.to_datetime(date_str, errors='coerce')
                    if pd.isna(dt):
                        dt = datetime.now()  # Default to current time if parsing fails
            except:
                dt = datetime.now()  # Default to current time if parsing fails
    ...
    def _extract_day_of_week(self, date_series):
        """Extract day of week from date with multiple format support"""
        days = []
        for date_str in date_series:
            try:
                # Try multiple date formats
...
def clean_dataset(df):
    """Clean the dataset by removing unnecessary columns and fixing data issues"""
    ...
    essential_cols = ['sender', 'receiver', 'date', 'subject', 'body', 'label', 'urls']
    available_cols = [col for col in essential_cols if col in df.columns]
    
    if len(available_cols) < len(essential_cols):
        print(f"⚠️ Missing columns: {set(essential_cols) - set(available_cols)}")
    
    df = df[available_cols]
    
    # Clean label column - convert to numeric and remove invalid entries
    print("Cleaning label column...")
    original_size = len(df)
    
    # Convert label to numeric, forcing errors to NaN
...
def select_numeric_features(X):
    """Select only numeric features for preprocessing"""
    numeric_features = X.select_dtypes(include=[np.number]).columns.tolist()
    print(f"✓ Selected {len(numeric_features)} numeric features for preprocessing")
...
def run_workflow(data_path='SpamAssasin.csv'):
    ...
    print(f"Class 0 samples: {len(df[df['label'] == 0])}")
    print(f"Class 1 samples: {len(df[df['label'] == 1])}")
    ...
    for col in X_features.columns:
        print(f"  {col}: {X_features[col].dtype}")
    ...
    if y.dtype == 'object':
        ...
        y_encoded = y.values
        target_names = ['Legitimate', 'Phishing']  # Assuming 0=Legitimate, 1=Phishing
        print(f"✓ Target variable already encoded. Classes: 0=Legitimate, 1=Phishing")
    ...
    # Select only numeric features for preprocessing
    X_train_numeric = select_numeric_features(X_train_balanced)
    X_test_numeric = select_numeric_features(X_test)
    ...
    # Apply preprocessing to numeric features only
    ...
    # Elastic Net Feature Selection - FIXED: Dynamic max_features
    print("\n--- Elastic Net Feature Selection ---")
    
    # Calculate dynamic max_features (can't exceed available features)
    n_features = X_train_processed.shape[1]
    max_features = min(25, n_features)  # Use 25 or available features, whichever is smaller
    
    print(f"Available features: {n_features}, Selecting: {max_features}")
    
    elastic_net = ElasticNet(alpha=0.5, l1_ratio=0.5, random_state=42, max_iter=10000)
    selector = SelectFromModel(elastic_net, max_features=max_features)
    ...
    print(f"✓ Features selected: {len(selected_features)} out of {X_train_numeric.shape[1]}")
    ...
    # Save the label encoder (if we created one)
    if 'le' in locals():
        with open('label_encoder.pkl', 'wb') as f:
            pickle.dump(le, f)
        print("✓ Label encoder saved to label_encoder.pkl")
    else:
        # Create and save a default label encoder
        le = LabelEncoder()
        le.fit(['Legitimate', 'Phishing'])
        with open('label_encoder.pkl', 'wb') as f:
            pickle.dump(le, f)
        print("✓ Default label encoder saved to label_encoder.pkl")
    ...
    print(f"🔧 Features Used: {len(selected_features)} out of {X_train_numeric.shape[1]}")
    print("=" * 60)
